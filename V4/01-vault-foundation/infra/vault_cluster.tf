...
locals {
  ...
  vault_api_addr = var.vault_instance_count > 1 ? (
    "http://${var.vault_lb_private_ip}:8200"  
  ) : "http://LOCAL_IP:8200"

  # cluster_addr → ALWAYS node-local IP:port (via placeholder)
  vault_cluster_addr = "http://LOCAL_IP:8201"

  leader_api_addr = var.vault_instance_count > 1 ? (
    "http://${var.vault_lb_private_ip}:8200"
  ) : "http://LOCAL_IP:8200"

  vault_hcl = templatefile("${path.module}/../cloud-init-vault/vault.hcl.tpl", {
  ...
  create_user_script = file("${path.module}/../cloud-init-vault/create-vault-user.sh")
  vault_service = file("${path.module}/../cloud-init-vault/vault.service")
  prereqs_script = file("${path.module}/../cloud-init-vault/install-prereqs.sh")
  install_vault_script = file("${path.module}/../cloud-init-vault/install-vault.sh")
}
...
# VMSS for Vault with System Assigned Managed Identity for Key Vault access
resource "azurerm_linux_virtual_machine_scale_set" "vault_vmss" {
  ...
  custom_data = base64encode(templatefile("${path.module}/../cloud-init-vault/user-data.yaml.tpl", {
    create_user_content   = indent(6, local.create_user_script)
    prereqs_content = indent(6, local.prereqs_script)
    install_vault_content = indent(6, local.install_vault_script)
