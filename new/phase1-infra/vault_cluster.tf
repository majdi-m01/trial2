...
locals {
  ...
  create_user_script = file("${path.module}/cloud-init/create-vault-user.sh")
  vault_service = file("${path.module}/cloud-init/vault.service")
  prereqs_script = file("${path.module}/cloud-init/install-prereqs.sh")
  install_vault_script = file("${path.module}/cloud-init/install-vault.sh")
}
